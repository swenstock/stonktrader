...
document.addEventListener('click',e=>{const btn=e.target.closest?.('#view-portfolio #queuedOrders button');if(!btn||!/CANCEL/i.test(clean(btn.textContent)))return;const row=btn.closest('article,tr,[data-order-id],div');const before=clean(row?.textContent||'');const symbol=(before.match(/\b[A-Z]{1,6}\b/g)||[]).find(x=>!['CANCEL','BUY','SELL','QUEUED','ORDER','AT','OPEN'].includes(x))||'';setTimeout(()=>{if(!document.contains(row)){add('CANCELLED',symbol,before.replace(/\bCANCEL\b/ig,'').slice(0,140)||'Queued order cancelled');}},120);},true);
...
window.addEventListener('sbc:working-orders-change',render);
