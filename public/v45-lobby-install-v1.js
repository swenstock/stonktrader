...
function progressDisplay(row){const progress=Number(row?.progress||0),redeem=Number(row?.redeemCount||20),toGo=Number(row?.juniorsToNextBroker);if(toGo===0&&progress===redeem)return'ELIGIBLE';if(toGo>=1&&toGo<=3)return`${toGo} TO GO`;return row?.progressLabel||`${progress} / ${redeem}`;}
function progressClass(row){const progress=Number(row?.progress||0),redeem=Number(row?.redeemCount||20),toGo=Number(row?.juniorsToNextBroker);if(toGo===0&&progress===redeem)return'rot-eligible';if(toGo===1)return'rot-near-1';if(toGo===2)return'rot-near-2';if(toGo===3)return'rot-near-3';return'';}
