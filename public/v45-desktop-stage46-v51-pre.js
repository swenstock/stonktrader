(()=>{
'use strict';
if(!window.matchMedia('(min-width:901px)').matches)return;
window.__sbcDesktopStage44V49=true;
window.__sbcDesktopStage47V52=true;
window.__sbcDesktopStage48V53=true;
window.__sbcDesktopStage49V54=true;
if(!document.querySelector('link[data-sbc-stage45-v63]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-desktop-stage45-v50.css?v=63';l.dataset.sbcStage45V63='1';document.head.appendChild(l);}
if(!window.SBCAnalyticsCoreV1&&!document.querySelector('script[data-sbc-analytics-core-v1]')){const s=document.createElement('script');s.src='/v45-analytics-core-v1.js?v=1';s.dataset.sbcAnalyticsCoreV1='1';document.head.appendChild(s);}
if(!document.querySelector('link[data-sbc-stage51-v55]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-desktop-stage51-v55.css?v=64';l.dataset.sbcStage51V55='1';document.head.appendChild(l);}
if(!document.querySelector('script[data-sbc-stage51-v55]')){const s=document.createElement('script');s.src='/v45-desktop-stage51-v55.js?v=63';s.dataset.sbcStage51V55='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('link[data-sbc-stage52-v56]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-desktop-stage52-v56.css?v=56';l.dataset.sbcStage52V56='1';document.head.appendChild(l);}
if(!document.querySelector('script[data-sbc-stage52-v56]')){const s=document.createElement('script');s.src='/v45-desktop-stage52-v56.js?v=56';s.dataset.sbcStage52V56='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('link[data-sbc-prize-info-v65]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-prize-info-v65.css?v=69';l.dataset.sbcPrizeInfoV65='1';document.head.appendChild(l);}
if(!document.querySelector('script[data-sbc-prize-info-v65]')){const s=document.createElement('script');s.src='/v45-prize-info-v65.js?v=70';s.dataset.sbcPrizeInfoV65='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('script[data-sbc-browser-history-v1]')){const s=document.createElement('script');s.src='/v45-browser-history-v1.js?v=1';s.dataset.sbcBrowserHistoryV1='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('link[data-sbc-desktop-stability-v1]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-desktop-stability-v1.css?v=2';l.dataset.sbcDesktopStabilityV1='1';document.head.appendChild(l);}
if(!document.querySelector('script[data-sbc-queued-activity-v1]')){const s=document.createElement('script');s.src='/v45-queued-activity-v1.js?v=1';s.dataset.sbcQueuedActivityV1='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('link[data-sbc-chart-workstation-v1]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/v45-chart-workstation-v1.css?v=2';l.dataset.sbcChartWorkstationV1='1';document.head.appendChild(l);}
if(!document.querySelector('script[data-sbc-chart-workstation-v1]')){const s=document.createElement('script');s.src='/v45-chart-workstation-v1.js?v=2';s.dataset.sbcChartWorkstationV1='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('script[data-sbc-advanced-chart-axis-lock-v1]')){const s=document.createElement('script');s.src='/v45-advanced-chart-axis-lock-v1.js?v=1';s.dataset.sbcAdvancedChartAxisLockV1='1';s.defer=true;document.head.appendChild(s);}
if(!document.querySelector('script[data-sbc-advanced-chart-visual-pass-v1]')){const s=document.createElement('script');s.src='/v45-advanced-chart-visual-pass-v1.js?v=1';s.dataset.sbcAdvancedChartVisualPassV1='1';s.defer=true;document.head.appendChild(s);}
})();
