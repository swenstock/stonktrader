...
const st={x:1,y:1,pan:0,wheel:0,drag:false,axisDrag:false,startX:0,startY:0,startPan:0,startScale:1,startResidual:0,dragPx:0,surface:null,view:null,raf:0};
...
function time(d){st.x=clamp(Math.round((st.x+d)*100)/100,.35,4);apply();repaint()}
...
function bind(card){
 ...
 const stop=()=>{if(st.drag||st.axisDrag){if(st.drag){const step=barStepPx(v);const combined=st.startPan*step+st.dragPx;const maxFuture=Math.max(80,v.getBoundingClientRect().width*.6);if(combined>=0){st.pan=Math.floor(combined/step);st.dragPx=combined-st.pan*step;}else{st.pan=0;st.dragPx=Math.max(-maxFuture,combined);}repaint();}st.drag=false;st.axisDrag=false;v.classList.remove('is-panning-v50','is-price-scaling-v50');apply()}};v.addEventListener('pointerup',stop);v.addEventListener('pointercancel',stop);v.addEventListener('lostpointercapture',stop);
...
window.SBCChartViewportV50={fit,price,time,apply,resetPrice,resetTime,repaintPrice:repaint,repaint,state:st};
