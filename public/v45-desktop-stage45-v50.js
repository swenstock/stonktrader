...
function apply(){if(!st.surface)return;st.pan=clamp(st.pan,-Math.max(0,(st.x-1)*180),Math.max(0,(st.x-1)*180));st.surface.style.transformOrigin='50% 50%';st.surface.style.transform=`translate3d(${st.pan}px,0,0) scaleX(${st.x})`;st.surface.style.transition=st.drag?'none':'transform .14s ease-out';const r=$('.stage45-readout-v50',root());if(r)r.textContent=`${Math.round(st.x*100)}% TIME • ${Math.round(st.y*100)}% PRICE`;}
function repaintPrice(){if(st.raf)return;st.raf=requestAnimationFrame(()=>{st.raf=0;if(typeof window.renderSymbolChart==='function')window.renderSymbolChart();const c=root(),s=c&&surface(c);if(s){st.surface=s;apply();}})}
function resetPrice(repaint=true){st.y=1;if(repaint)repaintPrice();}
function fit(){st.x=1;st.y=1;st.pan=0;st.wheel=0;apply();repaintPrice()}
function price(d){st.y=clamp(Math.round((st.y+d)*100)/100,.5,3);repaintPrice()}
...
function bind(card){
 ...
 v.addEventListener('pointermove',e=>{if(st.axisDrag){e.preventDefault();e.stopPropagation();const factor=Math.exp((st.startY-e.clientY)/220);st.y=clamp(Math.round(st.startScale*factor*100)/100,.5,3);repaintPrice();return;}if(st.drag){e.preventDefault();st.pan=st.startPan+(e.clientX-st.startX);apply();return;}v.classList.toggle('over-price-axis-v50',inPriceAxis(e,v));});
...
window.SBCChartViewportV50={fit,price,time,apply,resetPrice,repaintPrice,state:st};
