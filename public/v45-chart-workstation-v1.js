...
function dock(c=card()){return $('.stage43-chart-controls-v48',c)}
...
function installTopbar(c){
  ...
  $('[data-cw-fit]',bar).onclick=()=>{$('[data-v50-fit]',c)?.click();};
...
function installDrawingLayer(c){
  ...
  if(!$('.chart-drawing-rail-v1',v)){
    const rail=document.createElement('div');rail.className='chart-drawing-rail-v1';rail.innerHTML='<button type="button" data-cw-tool="pan" title="Pan / zoom">↔</button><button type="button" data-cw-tool="trend" title="Trend line">╱</button><button type="button" data-cw-tool="hline" title="Horizontal line">—</button><button type="button" data-cw-tool="measure" title="Measure">⌁</button><button type="button" data-cw-tool="crosshair" data-cw-native="CROSSHAIR" title="Crosshair">＋</button><button type="button" data-cw-clear title="Clear drawings">×</button>';v.appendChild(rail);$$('[data-cw-tool]',rail).forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();setTool(b.dataset.cwTool,c)});$('[data-cw-clear]',rail).onclick=e=>{e.preventDefault();e.stopPropagation();clearDrawings(v);setTool('pan',c)};
  }
