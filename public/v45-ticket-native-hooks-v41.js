...
function install(){
  ...
  if(typeof window.confirmBidOrder==='function'&&!window.confirmBidOrder.__sbcV41){const original=window.confirmBidOrder;const wrapped=function(){let name=currentName(),price=Number(document.getElementById('bidOrderPrice')?.value||0),best=0,ask=Infinity;try{name=String(bidOrder?.name||name);best=Number(bidOrder?.bestBid||0);ask=Number(bidOrder?.ask||Infinity)}catch(_){}const result=original.apply(this,arguments);if(!(price>best&&price<ask))return result;const key=`bid:${name}:${price}`;if(!dedupe(key))return result;ensureLocalOrder('bid',name,price);insertPrice('bid',price);redraw();return result;};wrapped.__sbcV41=true;window.confirmBidOrder=wrapped;}
