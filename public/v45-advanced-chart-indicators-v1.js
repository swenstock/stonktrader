...
(() => {
  ...
  function setup(api,overlay){
    ...
    const replaceActive=ids=>{active.clear();for(const id of ids||[]){const item=CATALOG.find(x=>x.id===id);if(item?.implemented)active.add(id);}if(panelOpen)drawList();render();};
    ...
    const button=document.createElement('button');button.dataset.action='indicators';button.title='Indicators';button.setAttribute('aria-label','Indicators');button.innerHTML='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M4 12h3l2-6 4 12 2-6h5"/></svg>';const grow=toolbar.querySelector('.grow');toolbar.insertBefore(button,grow);
    ...
    list.addEventListener('click',e=>{const fav=e.target.closest('[data-fav]');if(fav){const id=fav.dataset.fav;favorites.has(id)?favorites.delete(id):favorites.add(id);localStorage.setItem('sbcChartIndicatorFavorites',JSON.stringify([...favorites]));drawList();return;}const b=e.target.closest('[data-ind]');if(!b||b.disabled)return;active.has(b.dataset.ind)?active.delete(b.dataset.ind):active.add(b.dataset.ind);drawList();render();api.notifyLayoutChanged?.();});
    api.registerIndicatorPersistence?.({getActive:()=>[...active],setActive:replaceActive});
