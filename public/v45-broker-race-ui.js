...
const PROMOTION_COPY=Object.freeze({
  ...
  heroSupport:'COMPETE. WIN. COLLECT JR. STONK BROKER BADGES.<br>COLLECT 20. GET PROMOTED.',
  raceCopy:'Climb the corporate ladder. Collect Jr. Stonk Broker Badges. Collect 20 and get promoted to an Activated StonkBroker. Your personal collection lives in My Contests; this board tracks the race across all SBC players.',
...
function progressText(r){if(Number(r.juniorCount||0)>=Number(r.redeemCount||20)&&Number(r.progress||0)===Number(r.redeemCount||20))return 'READY FOR PROMOTION';const left=Number(r.juniorsToNextBroker||0);return left===1?'1 BADGE TO PROMOTION':`${left} BADGES TO PROMOTION`}
function topRows(rows,limit=5){const list=rows.slice(0,limit);if(!list.length)return'<div class="sbc-race-empty">No Jr. Stonk Broker Badges have been collected yet. The race starts with the first award.</div>';return list.map(r=>`<div class="sbc-race-row"><div class="sbc-race-rank">#${r.rank}</div><div class="sbc-race-name"><b>${esc(r.displayName)}</b><span>${esc(progressText(r))}</span></div><div class="sbc-race-count">${r.juniorCount} BADGES</div></div>`).join('')}
...
function renderHome(model){
  ...
  panel.innerHTML=`<div class="sbc-race-kicker">PLATFORM-WIDE PROGRESSION</div><div class="sbc-race-title">THE BROKER RACE</div><div class="sbc-race-layout"><div><div class="sbc-race-stats"><div class="sbc-race-stat"><small>STONKBROKERS EARNED</small><b>${model.brokersEarned}</b><span>Lifetime funded redemptions</span></div><div class="sbc-race-stat"><small>JR. STONK BROKER BADGES AWARDED</small><b>${model.juniorsAwarded}</b><span>Lifetime contest + mint issuance</span></div><div class="sbc-race-stat"><small>JR. STONK BROKER BADGES COLLECTED</small><b>${model.juniorsStacked}</b><span>Currently held in player collections</span></div></div><p class="sbc-race-copy">${PROMOTION_COPY.raceCopy}</p></div><div class="sbc-race-top"><div class="sbc-race-top-head"><b>${PROMOTION_COPY.topTitle}</b><span>${PROMOTION_COPY.topLabel}</span></div>${topRows(model.topStackers,5)}</div></div>`;
  return true;
}
...
function patchCorporateLadderCopy(){
  ...
  if(statement){const cells=[...statement.children];if(cells[0])cells[0].innerHTML='<b>🏆 CLIMB THE LADDER</b><span>Paid contests protect the top 10%. Fully funded Badges go first; remaining places receive the tier fallback.</span>';if(cells[2])cells[2].innerHTML='<b>👑 GET PROMOTED</b><span>Collect 20 Jr. Stonk Broker Badges. Get promoted to an Activated StonkBroker.</span>';}
  const steps=$$('#how .step');
  if(steps[2]){const h=steps[2].querySelector('h3');const ps=steps[2].querySelectorAll(':scope>p');if(h)h.textContent='FINISH TOP 10%';if(ps[0])ps[0].textContent='Paid contests target the top 10%. Fully funded Jr. Stonk Broker Badges go to the highest eligible finishers; remaining places receive the tier fallback.';if(ps[1])ps[1].innerHTML='<strong>Tickets:</strong> play them, hold them, sell them, or collect 10 for an optional upgrade.';}
  if(steps[3]){const h=steps[3].querySelector('h3');const ps=steps[3].querySelectorAll(':scope>p');if(h)h.textContent='GET PROMOTED';if(ps[0])ps[0].textContent='Collect Jr. Stonk Broker Badges as you compete. Collect 20 and get promoted to an Activated StonkBroker.';if(ps[1])ps[1].innerHTML='Your Badge collection follows you across SBC — it is <strong>not</strong> tied to one contest.';const fan=steps[3].querySelector('.fanfare');if(fan)fan.textContent='🏆 COLLECT 20 JR. STONK BROKER BADGES → GET PROMOTED.';}
  const footer=$('.footer-card');if(footer)footer.innerHTML='<strong>CLIMB THE CORPORATE LADDER.</strong><br><span>Win tickets. Collect Badges. Collect 20. Get promoted.</span>';
}
