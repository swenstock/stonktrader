(()=>{
  ...
  function ensureLaunchButton(){
    ...
    const btn=document.createElement('button');btn.type='button';btn.className='quick-ticket-launch';btn.innerHTML='<span>⚡</span> QUICK TICKET';
  ...
  function openCountPrompt(){
    ...
    $('#qtBody',modal).innerHTML=`<section class="qt-count-step"><div class="qt-step-num">1</div><div><h3>How many stocks will you be buying?</h3><p>Minimum ${MIN}. Quick Ticket equal-weights them automatically.</p></div><input id="qtCount" type="number" min="${MIN}" max="${MAX}" value="10"><button id="qtBuild" type="button">BUILD TICKET</button></section>`;
  ...
  function renderEditor(){
    ...
    $('#qtBody',modal).innerHTML=`<section class="qt-editor">
      ...
      <div class="qt-editor-foot"><button type="button" class="ghost" id="qtBackCount">CHANGE COUNT</button><span>Equal-weight target • Server rules still apply</span><button type="button" class="primary" id="qtReview">REVIEW QUICK TICKET →</button></div>
    </section>`;
  ...
  async function validateRows(){
    ...
    const unique=new Set(symbols);if(unique.size!==symbols.length)throw new Error('Each stock may appear only once in a Quick Ticket.');
  ...
  async function review(){
    ...
    try{
      ...
      if(!portfolio.tradingAllowed)throw new Error(portfolio.tradingMessage||'Trading is currently closed.');
      ...
      state.review={portfolio,rows:desired,weight:preview.weight,scale,totalSpend:desired.reduce((s,r)=>s+r.spend,0)};state.step='review';renderReview();
    }catch(e){alert(e.message);renderEditor();}
  }

  function renderReview(){
    const rv=state.review;$('#qtSave',modal).disabled=false;
    $('#qtBody',modal).innerHTML=`<section class="qt-review"><div class="qt-editor-top"><div class="qt-step-num">3</div><div><h3>Review Quick Ticket</h3><p>${rv.rows.length} stocks • ${rv.weight.toFixed(2)}% target weight each • ${money(rv.totalSpend)} estimated new buys</p></div><div class="qt-allocation">AVAILABLE CASH <b>${money(rv.portfolio.cash)}</b></div></div>
      ...
      <footer class="qt-submit-foot"><button type="button" class="ghost" id="qtEdit">← EDIT TICKET</button><div><small>Orders execute at the current simulated market quote and are rechecked against SBC rules.</small><b>${rv.rows.filter(r=>r.spend>.01).length} BUY ORDERS</b></div><button type="button" class="primary" id="qtSubmit">SUBMIT QUICK TICKET</button></footer></section>`;
  ...
  async function saveCurrentList(){
    ...
    const name=prompt('Name this saved Quick Ticket list:');if(!name?.trim())return;
  ...
  async function showSavedLists(){
    try{
      ...
      $('#qtBody',modal).innerHTML=`<section class="qt-saved"><div class="qt-editor-top"><div class="qt-step-num">★</div><div><h3>Saved Quick Ticket Lists</h3><p>Load a basket and its equal weights will be recalculated for this portfolio.</p></div></div><div class="qt-saved-list">${lists.length?lists.map(x=>`<div class="qt-saved-row"><button class="qt-load-list" data-id="${x.id}"><b>${esc(x.name)}</b><span>${x.symbols.length} stocks • ${x.symbols.slice(0,5).join(', ')}${x.symbols.length>5?'…':''}</span></button><button class="qt-delete-list" data-id="${x.id}" title="Delete saved list">DELETE</button></div>`).join(''):'<div class="qt-empty">No saved Quick Ticket lists yet.</div>'}</div><footer class="qt-editor-foot"><button type="button" class="ghost" id="qtNewList">+ NEW QUICK TICKET</button></footer></section>`;
