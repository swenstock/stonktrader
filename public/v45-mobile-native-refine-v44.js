...
function killSymbolSheet(e){const close=e?.target?.closest?.('#sbcM43SymbolSheet .close');if(!close)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();$('#sbcM43SymbolSheet')?.remove();}
...
/* Window capture runs before legacy document capture handlers in the exact V45 shell. */
window.addEventListener('click',killSymbolSheet,true);
