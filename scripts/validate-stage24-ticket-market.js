...
must(ui.includes('window.__SBC_TICKET_REAL_OFFER_TEST?.renderAuthoritativeMyTickets?.()')&&hooks.includes("await fetch('/api/tickets'")&&hooks.includes('ticket.id')&&!hooks.includes('MY-ME-1'),'My Tickets delegates from V36 to V41 authoritative backend inventory with real ticket ids');
